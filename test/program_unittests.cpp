...
TEST_CASE("recreate simple commands", "[recreate]")
{
    ...
    {
        compiler.addNumConstInstruction(true, "-1.23e45", 0);
...
TEST_CASE("miscellaneous error class coverage", "[misc-coverage]")
{
    ...
    SECTION("cover dynamically allocated expected expression error class")
    {
        std::unique_ptr<ExpExprError> error {new ExpExprError {0}};
    }
}
